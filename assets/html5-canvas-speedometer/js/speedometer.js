window.Dashboard = {
    draw: function( canvas, speed, tacho, gas, mileage, turnSignalsStates, iconsStates )
    {
        draw(canvas, speed, tacho, gas, mileage, turnSignalsStates, iconsStates);
    }
}
...
class Circle {
    ...
    drawSupport() {
        ...
        this.ctx.lineTo(this.x, canvas.height);
        this.ctx.moveTo(0, this.y);
        this.ctx.lineTo(canvas.width, this.y);
        this.ctx.closePath();
    ...
    drawUnit(offsetY) {
        ...
        this.ctx.font = `${this.markFontStyle} ${this.markFontSize}px ${this.markFontFamily}`;
        this.ctx.fillStyle = this.markFontColor;
        this.ctx.fillText(value, -this.ctx.measureText(value).width / 2, offsetY - 2);
        this.ctx.restore();
    }
...
class AdditionalCircle extends Circle {
     ...
     */
    constructor(canvas, options, side, mainCircleRadius) {
        super(canvas, options);
 ...
 */
function drawLeftCircle(canvas, options, radius, value, iconStates) {
    const lCircle = new AdditionalCircle(canvas, options, 'left', radius);
    lCircle.draw();
 ...
 * @param {Object} iconStates States of icons displayed on circle.
 */
function drawRightCircle(canvas, options, radius, value, iconStates) {
    const rCircle = new AdditionalCircle(canvas, options, 'right', radius);
    rCircle.draw();
 ...
 */
function draw(canvas, speedometerValue, tachometerValue, gasValue, mileage, turnSignals, iconStates) {
    clearCanvas(canvas);

    const mCircle = new MainCircle(canvas,options);
    drawLeftCircle(canvas, options, mCircle.radius, tachometerValue, iconStates);
    drawRightCircle(canvas, options, mCircle.radius, gasValue, iconStates);
